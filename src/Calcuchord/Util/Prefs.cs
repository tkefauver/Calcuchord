...
public class Prefs : ViewModelBase {
    ...
    private static bool IsParsing { get; set; }

    public static Prefs Parse(string prefsJson) {
        IsParsing = true;
        var result = JsonConvert.DeserializeObject<Prefs>(prefsJson);
        IsParsing = false;
        return result;
    }

    private static void DoModelUpToDateCheck() {
        try {
            var backup_prefs_json =
                MpAvFileIo.ReadTextFromResource("avares://Calcuchord/Assets/Text/appstate_022325.json");
            var ValidationChecker = Parse(backup_prefs_json);
            Instance = null;
        }
        catch (Exception ex) {
            ex.Dump();
    ...
    public static async Task InitAsync() {
        ...
        if (RESET_PREFS) await prefsIo.WritePrefsAsync(string.Empty);
#if DEBUG
        DoModelUpToDateCheck();
#endif

        var prefs_json = await prefsIo.ReadPrefsAsync();

        var is_initial_startup = string.IsNullOrEmpty(prefs_json);
    ...
    [JsonProperty] public bool IsExactMatchOnly { get; set; }

    [JsonProperty] public string PrefsVersion { get; set; } = string.Empty;

    [JsonProperty] public bool IsThemeDark { get; set; }

    [JsonProperty] public int MatchColCount { get; set; } = MainViewModel.DEFAULT_MATCH_COL_COUNT;


    [JsonProperty] public List<Instrument> Instruments { get; set; } = [];

    [JsonProperty] public List<OptionViewModel> Options { get; set; } = [];

    [JsonProperty] public List<BookmarkGroup> BookmarkGroups { get; set; } = [];

    #endregion

    #region Ignored

    [JsonIgnore] public bool WasOptionsOutOfDateOnStartup { get; private set; }

    [JsonIgnore] public bool IsOptionsRequireReset { get; private set; }


    [JsonIgnore] public string LastPrefsVersion { get; private set; } = string.Empty;

    [JsonIgnore] private BuildInfo RuntimeBuildInfo { get; } = new();

    [JsonIgnore] public Version LastOptionsUpdatedPrefsVersion { get; } = new("1.0.9214.18195");

    [JsonIgnore] private Version ResetRequiredPrefsVersion { get; } = new("1.0.9195.22711");

    [JsonIgnore] public bool IsSaveIgnored { get; set; }

    [JsonIgnore] public bool IsInitialStartup { get; private set; }

    [JsonIgnore] private static bool RESET_PREFS => false;
    ...
    private DateTime LastSaveCalledDt { get; set; }

    public void Save(bool immediate = false) {
        var call_time = DateTime.Now;
        ...
        Task.Run(
            async () => {
                ...
                while (true) {
                    ...
                    if (DateTime.Now - call_time > TimeSpan.FromSeconds(10) || immediate) break;
...
#endif
                try {
                    var pref_json = JsonConvert.SerializeObject(this);
    ...
    private void SyncModels() {
        ...
        if (ThemeViewModel.Instance is { } tvm) IsThemeDark = tvm.IsDark;
    }

    private void LogPrefs() {
        PlatformWrapper.Services.Logger.WriteLine("");
        var tuning_str = MainViewModel.Instance == null || MainViewModel.Instance.SelectedTuning == null
            ? string.Empty
            : MainViewModel.Instance.SelectedTuning.ToString();
        var sel_tuning_full_name = "NONE";
        ...
        foreach (var tuning in inst.Tunings)
            PlatformWrapper.Services.Logger.WriteLine(
