...
public class ChordSvgBuilder : SvgBuilderBase {
    public override HtmlNode Build(NotePattern ng,object args) {
        var svg = InitBuild(args);

        var cntr_g = CurrentDoc.CreateElement("g");
        svg.AppendChild(cntr_g);

        var fret_grid_g = CurrentDoc.CreateElement("g");
        cntr_g.AppendChild(fret_grid_g);

        var bg_g = CurrentDoc.CreateElement("g");
        ...
        int nut_fret_num = ng.Parent.Parent.CapoFretNum;
        int min_fret = nut_fret_num;
        int min_vis_fret = min_fret;
        int max_fret = min_fret;
        // get min/max visual frets
        if(notes.Where(x => x.ColNum >= nut_fret_num) is { } real_frets &&
           real_frets.Any()) {
            min_fret = real_frets.Min(x => x.ColNum);
            max_fret = real_frets.Max(x => x.ColNum);
            if(real_frets.Where(x => x.ColNum > nut_fret_num) is { } vis_frets &&
               vis_frets.Any()) {
        ...
        bool show_header_labels = notes.Any(x => x.ColNum <= nut_fret_num);
        bool show_nut = max_fret - nut_fret_num < vfc;
        bool show_fret_marker = !show_nut;
        if(show_nut) {
            min_fret = nut_fret_num;
            min_vis_fret = nut_fret_num + 1;
        }
        ...
        // fret labels
        {
            var fret_g = CreateG(bg_g,"fret-labels");
        ...
        if(show_fret_marker) {
            ...
            // open label
            var marker_g = CreateG(bg_g,"fret-marker");
        ...
        if(show_header_labels) {
            ...
            for (int i = 0; i < str_count; i++) {
                if(notes.FirstOrDefault(x => x.RowNum == i) is { } str_fret &&
                   str_fret.ColNum <= nut_fret_num) {
                    bool is_mute = str_fret.ColNum < nut_fret_num;
                    if(is_mute) {
                        double mute_size = 3;
                        double margin_y = 1;
        ...
        // tuning
        {
            ...
            var tuning_g = CreateG(bg_g,"string-tuning");
        ...
        for (int vis_fret_num = 0; vis_fret_num < vfc; vis_fret_num++) {
            ...
            for (int str_num = 0; str_num < str_count; str_num++) {
                ...
                var fret_note = notes.Where(x => x.ColNum > nut_fret_num)
                    .FirstOrDefault(x => x.RowNum == str_num && x.ColNum == fret_num);
                string fret_bg = null;
                string fret_fg = null;

                var primary_note = fret_note ?? last_barre_note;
                ...
                if(str_count == 1 || str_num < str_count - 1) {
                    ...
                    if(fret_num == nut_fret_num + 1 && show_nut) {
                        // nut line
                ...
                if(barred_fret_lookup.TryGetValue(fret_num,out var bar_extent) &&
                   ...
                   shadow_barre_fret_num.Value == fret_num) {
                    ...
                    if(fret_note == null &&
                       (str_num == 0 || str_num == str_count - 1)) {
                        // shadow edge
                        var shadow_g = CurrentDoc.CreateElement("g");
                        ...
                        var shadow_path = CurrentDoc.CreateElement("path");
                ...
                if(fret_note != null) {
                    ...
                    void AddFingerShape(bool is_box,string classes) {
                        ...
                        var shape_cntr = CreateG(bg_g,classes);
                        if(is_box) {
                            // root outer box
