...
public class MainViewModel : ViewModelBase {
    ...
    private async Task InitAsync(IEnumerable<Instrument> instl = null) {
        ...
        foreach (var inst in instl) {
            var ivm = await CreateInstrumentAsync(inst);
            Instruments.Add(ivm);
        }
        ...
        double GetSortOptionValue((MatchSortType field, bool desc) sort, MatchViewModel match) {
            ...
            switch (sort.field) {
                ...
                case MatchSortType.Score:
                    score = IsSearchModeSelected ? match.Score : 1;
                    break;
            }
    ...
    public ICommand ExportMatchesCommand => new MpCommand<object>(
        async args => {
            ...
            var busy_view = new ProgressView();
    ...
    public ICommand FinishTranslateCommand => new MpCommand(
        () => {
            ...
            var source_pattern = TranslateSourceMatchViewModel.NotePattern;
